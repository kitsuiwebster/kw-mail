...
from app.tools import search_emails, get_full_email, TOOL_DEFINITIONS, execute_tool
...
def main():
    ...
    try:
        ...
        # Test query that should trigger search_emails
        test_query = """L'utilisateur te demande: "Montre-moi les emails de l'INPI"

Utilise l'outil search_emails pour chercher les emails contenant "INPI"."""

        print(f"Query: \"Montre-moi les emails de l'INPI\"")
        print()
        ...
        if results:
            email_id = results[0]['id']
            test_query_2 = f"""L'utilisateur demande le contenu complet d'un email spécifique.

L'ID de l'email est: {email_id}

Utilise l'outil get_full_email pour récupérer le contenu complet, puis résume-le."""

            print(f"Query: \"Donne-moi le contenu complet de l'email INPI\"")
            print()
